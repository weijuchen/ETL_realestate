...
def main():
    spark=SparkSession.builder.appName("ReadMinioCsv").getOrCreate()
    

    # spark=SparkSession.builder.appName("RealestateETL").master("spark://spark-master:7077").getOrCreate()
    # MinIO 設定 (很重要)
    # spark._jsc.hadoopConfiguration().set("fs.s3a.endpoint", "http://minio:9000")
    # spark._jsc.hadoopConfiguration().set("fs.s3a.access.key", "minio123")
    # spark._jsc.hadoopConfiguration().set("fs.s3a.secret.key", "minio123")
    # spark._jsc.hadoopConfiguration().set("fs.s3a.path.style.access", "true")
    # spark._jsc.hadoopConfiguration().set("fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem")

    print(f"Endpoint: {spark.sparkContext._jsc.hadoopConfiguration().get('fs.s3a.endpoint')}")

# 讀 CSV
    s3_path="s3a://realestate-market/real_estate1141/a_lvr_land_a.csv"
    df = spark.read \
        .option("header", "true") \
        .csv(s3_path)
